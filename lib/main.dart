...
const _tIcons   = [Icons.water_drop_rounded, Icons.auto_awesome_rounded, Icons.waves_rounded, Icons.favorite_rounded, Icons.wb_sunny_rounded];
...
BoxDecoration get _bgDecor => const BoxDecoration(
  gradient: LinearGradient(colors: [AppColors.bg, Color(0xFF0A0E1A)], begin: Alignment.topLeft, end: Alignment.bottomRight),
);
...
Widget _primaryBtn({required String label, required VoidCallback? onTap, Color? color, IconData? icon}) {
  final c = color ?? _ac;
  return SizedBox(
    width: double.infinity, height: 52,
    child: DecoratedBox(
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [c, c.withBlue((c.blue + 40).clamp(0, 255))]),
        borderRadius: BorderRadius.circular(14),
        boxShadow: [BoxShadow(color: c.withOpacity(0.35), blurRadius: 16, offset: const Offset(0, 6))],
      ),
...
Widget _ghostBtn({required String label, required VoidCallback? onTap, Color? color, IconData? icon}) {
  final c = color ?? AppColors.textSub;
  return SizedBox(
    width: double.infinity, height: 44,
    child: OutlinedButton.icon(
      style: OutlinedButton.styleFrom(
        foregroundColor: c, side: BorderSide(color: c.withOpacity(0.35)),
...
Widget _sectionHeader(IconData icon, Color color, String label) => Row(children: [
  ...
  Text(label.toUpperCase(), style: TextStyle(color: color, fontSize: 11, letterSpacing: 1.4, fontWeight: FontWeight.w700)),
]);
...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Future<void> _connectToMachine() async {
    ...
    try {
      bool ok = await WiFiForIoTPlugin.connect("ESP32_VENDO",
        password: "12345678", security: NetworkSecurity.WPA, joinOnce: true, withInternet: false);
      if (ok) {
        await WiFiForIoTPlugin.forceWifiUsage(true);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: const Text("Connected! Scanning for nearby WiFi…"), backgroundColor: _alt));
          await _scanNearbyWiFi();
          _goNext();
        }
      } else {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("Could not connect. Is the machine turned on?"), backgroundColor: AppColors.danger));
      }
    } catch (e) { debugPrint("WiFi error: $e"); }
  ...
  Future<void> _scanNearbyWiFi() async {
    ...
    try {
      ...
      if (status.isGranted) {
        ...
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("Location permission required to scan WiFi."), backgroundColor: AppColors.warn));
      }
    } catch (e) { debugPrint("Scan error: $e"); }
    finally { if (mounted) setState(() => _isScanningWifi = false); }
  }
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Row(children: [
                  Container(
                    ...
                    decoration: BoxDecoration(color: _ac.withOpacity(0.12), borderRadius: BorderRadius.circular(8)),
  ...
  Widget _page1() => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
      ...
      const Text("Link to Machine", textAlign: TextAlign.center,
      ...
      const Text("Keep your phone near the Vendo Machine.\nWe will connect to it automatically.",
          textAlign: TextAlign.center, style: TextStyle(color: AppColors.textSub, height: 1.5)),
      ...
      Container(
        ...
        child: Row(children: [
          Container(width: 40, height: 40,
            decoration: BoxDecoration(color: _ac.withOpacity(0.1), shape: BoxShape.circle),
          ...
          Container(
            ...
            decoration: BoxDecoration(color: _alt.withOpacity(0.1), borderRadius: BorderRadius.circular(6)),
            child: Text("READY", style: TextStyle(color: _alt, fontSize: 11, fontWeight: FontWeight.bold))),
        ]),
      ),
      const SizedBox(height: 32),
      _isConnecting
        ? Column(children: [CircularProgressIndicator(color: _ac), const SizedBox(height: 10),
      ...
      const SizedBox(height: 12),
      _primaryBtn(label: "Add QR Code (Temp)", color: AppColors.warn, icon: Icons.qr_code_2_rounded,
  ...
  Widget _page2() => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const Center(child: Text("Select your home WiFi so the machine\ncan connect to the cloud.",
          textAlign: TextAlign.center, style: TextStyle(color: AppColors.textSub, height: 1.5))),
      ...
      if (_isScanningWifi)
        Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: _ac)),
            const SizedBox(width: 12),
        ...
        Container(
          ...
          child: DropdownButtonHideUnderline(child: DropdownButton<String>(
            ...
            items: _scannedSSIDs.map((s) => DropdownMenuItem(value: s,
              child: Row(children: [Icon(Icons.wifi_rounded, color: _ac, size: 18), const SizedBox(width: 8),
                Text(s, style: const TextStyle(color: AppColors.textPrim))]))).toList(),
        ...
        Container(
          ...
          child: Column(children: [
            const Row(children: [Icon(Icons.signal_wifi_off, color: AppColors.danger, size: 20), SizedBox(width: 8),
              Expanded(child: Text("No networks found. Enable Location and retry.",
                style: TextStyle(color: AppColors.textSub, fontSize: 13)))]),
            const SizedBox(height: 12),
            _ghostBtn(label: "Rescan Networks", onTap: _scanNearbyWiFi, icon: Icons.refresh_rounded, color: _ac),
          ])),
      const SizedBox(height: 16),
      _label("WiFi Password"), const SizedBox(height: 8),
      TextField(
        controller: _passCtrl, obscureText: true,
        style: const TextStyle(color: AppColors.textPrim),
        decoration: InputDecoration(
          hintText: "Enter password", hintStyle: const TextStyle(color: AppColors.textSub),
          filled: true, fillColor: AppColors.card,
          prefixIcon: const Icon(Icons.lock_outline_rounded, color: AppColors.textSub),
          enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: AppColors.divider)),
          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: _ac)),
        ),
      ),
      const SizedBox(height: 28),
      _primaryBtn(label: "Send to Machine", icon: Icons.send_rounded, color: AppColors.warn, onTap: () async {
        if (_selectedSSID == null || _passCtrl.text.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text("Please select a WiFi and enter the password."), backgroundColor: AppColors.warn)); return;
        }
        try {
          var res = await http.post(Uri.parse('http://192.168.4.1/setup'),
            body: {"ssid": _selectedSSID, "pass": _passCtrl.text}).timeout(const Duration(seconds: 7));
          if (!context.mounted) return;
          if (res.statusCode == 200) {
            await WiFiForIoTPlugin.forceWifiUsage(false);
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: const Text("Credentials sent! Machine is rebooting."), backgroundColor: _alt)); _goNext();
          } else {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text("Machine rejected the data."), backgroundColor: AppColors.danger));
          }
        } catch (e) {
          await WiFiForIoTPlugin.forceWifiUsage(false);
          if (!context.mounted) return;
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text("TRUE ERROR: ${e.toString()}"), backgroundColor: AppColors.danger,
  ...
  Widget _page3() => Column(children: [
    Padding(
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 16),
        _label("Step 3 of 3"), const SizedBox(height: 6),
        const Text("Link Machine", style: TextStyle(fontSize: 26, fontWeight: FontWeight.w800, color: AppColors.textPrim)),
        const SizedBox(height: 8),
              ...
              if (code != null && code != _qrResult) {
                ...
                try {
                  ...
                  if (event.snapshot.value != null) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                      content: Text("Unsuccessful login: QR not recognized"), backgroundColor: AppColors.danger));
                  }
                } catch (e) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text("Error checking QR: $e"), backgroundColor: AppColors.danger));
                }
              }
            }
          }),
    ...
    Padding(
      ...
      child: Column(children: [
        _primaryBtn(label: "Simulate QR Scan & Finish", color: _alt,
            icon: Icons.check_circle_outline_rounded, onTap: _finishSetup),
        const SizedBox(height: 10),
...
/// Wraps [_ScanOverlayPainter] in an animated widget so the scan
...
class _ScanOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    const margin     = 40.0; // inset of the clear scanning window
    const cornerLen  = 28.0;
    const cornerR    = 6.0;
    const strokeW    = 3.0;

    final winRect = Rect.fromLTRB(margin, margin, size.width - margin, size.height - margin);
    ...
    // Re-draw hole using Even-Odd rule
    canvas.drawPath(
      overlayPath..fillType = PathFillType.evenOdd,
      Paint()..color = Colors.black.withOpacity(0.55)..style = PaintingStyle.fill,
    );

    // ── 2. Window border (subtle) ──────────────────────────────
    canvas.drawRRect(
    ...
    drawCorner(tl, Offset(cornerLen, 0), Offset(0, cornerLen));
    drawCorner(tr, Offset(-cornerLen, 0), Offset(0, cornerLen));
    drawCorner(bl, Offset(cornerLen, 0), Offset(0, -cornerLen));
    drawCorner(br, Offset(-cornerLen, 0), Offset(0, -cornerLen));
    ...
    canvas.drawLine(Offset(winRect.left + 8, lineY), Offset(winRect.right - 8, lineY), linePaint);
...
class AddQrPage extends StatefulWidget {
  ...
  @override State<AddQrPage> createState() => _AddQrPageState();
}
...
class _AddQrPageState extends State<AddQrPage> {
  ...
  void _handleScan(String raw) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text("QR code decrypted and saved"), backgroundColor: _alt));
      Navigator.pop(context);
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text("Security Alert: Invalid or fake QR Code scanned!"), backgroundColor: AppColors.danger));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new_rounded, color: AppColors.textSub),
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              ...
              decoration: BoxDecoration(color: AppColors.warn.withOpacity(0.08),
                borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.warn.withOpacity(0.3))),
              child: const Row(children: [
                Icon(Icons.info_outline_rounded, color: AppColors.warn, size: 18), SizedBox(width: 8),
            ...
            Expanded(child: ClipRRect(
              ...
              child: Stack(children: [
                MobileScanner(controller: _ctrl, onDetect: (capture) {
                  ...
                  if (capture.barcodes.isNotEmpty) {
                    final code = capture.barcodes.first.rawValue;
                    if (code != null) { setState(() => _isProcessing = true); _handleScan(code); }
                  }
                }),
                Positioned.fill(child: ScanOverlay(_alt)),
                if (_isProcessing) Container(color: Colors.black54,
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Map<String, dynamic> currentSettings = {
    "1": {"enabled": true, "time": 5}, "5": {"enabled": true, "time": 15},
    "10": {"enabled": true, "time": 30}, "20": {"enabled": true, "time": 60},
  };
    ...
    return Scaffold(
      ...
      body: StreamBuilder(
        ...
        builder: (context, AsyncSnapshot<DatabaseEvent> snap) {
          ...
          if (!snap.hasData || snap.data?.snapshot.value == null) return _emptyState();
          ...
          // Live status values (ESP32 writes these to /status/)
          ...
          return Container(
            ...
            child: ListView(padding: const EdgeInsets.all(20), children: [
              ...
              const SizedBox(height: 24),
              _primaryBtn(label: "View Daily Logs", icon: Icons.bar_chart_rounded,
  ...
  Widget _emptyState() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    _glowIcon(Icons.cloud_off_rounded, AppColors.textSub), const SizedBox(height: 16),
  ...
  Widget _heroCard(double total) => Container(
    ...
    decoration: BoxDecoration(
      gradient: LinearGradient(colors: [_alt.withOpacity(0.85), _alt.withOpacity(0.3)],
        begin: Alignment.topLeft, end: Alignment.bottomRight),
      borderRadius: BorderRadius.circular(20),
      boxShadow: [BoxShadow(color: _alt.withOpacity(0.3), blurRadius: 24, offset: const Offset(0, 8))],
    ),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Row(children: [
        Icon(Icons.account_balance_wallet_rounded, color: Colors.white70, size: 18), SizedBox(width: 6),
  ...
  // ── Live Stats: WiFi Users + Active Charging Ports ──────────
  ...
  Widget _statBox({required IconData icon, required Color color, required String label, required String value, required String sub}) =>
    Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(
          colors: [AppColors.card, color.withOpacity(0.06)],
          begin: Alignment.topLeft, end: Alignment.bottomRight),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(value, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: AppColors.textPrim, height: 1)),
        const SizedBox(height: 4),
        Text(sub, style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 0.5)),
  ...
  void _showSettingsDialog() {
    ...
    bool dlgScanning = false;
    List<String> dlgSSIDs = [];
    String? dlgSSID;

    Future<void> scanDlgWifi(Function setDlg) async {
      setDlg(() => dlgScanning = true);
      try {
        ...
        if (status.isGranted) {
          ...
          if (canScan == CanStartScan.yes) {
            ...
            final results = await WiFiScan.instance.getScannedResults();
            final unique = results.map((n) => n.ssid).where((s) => s.isNotEmpty).toSet().toList();
            setDlg(() { dlgSSIDs = unique; if (unique.isNotEmpty) dlgSSID = unique.first; });
          }
        }
      } catch (e) { debugPrint("Scan error: $e"); }
      finally { setDlg(() => dlgScanning = false); }
    }

    Future<void> sendWifi(Function setDlg, BuildContext ctx) async {
      if ((dlgSSID == null || dlgSSID!.isEmpty) || passCtrl.text.isEmpty) {
        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(
          content: Text("Select a network and enter the password."), backgroundColor: AppColors.warn)); return;
      }
      setDlg(() => isSending = true);
      try {
        await _db.child("commands/wifi_change").set({"ssid": dlgSSID, "pass": passCtrl.text, "pending": true});
        if (ctx.mounted) Navigator.pop(ctx);
        if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Text("✓ WiFi change sent! Machine will reboot shortly."),
          backgroundColor: _alt, duration: const Duration(seconds: 4)));
      } catch (e) {
        setDlg(() => isSending = false);
        if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(
          content: Text("Failed to send: $e"), backgroundColor: AppColors.danger));
      }
    }
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                Container(padding: const EdgeInsets.all(8),
                  decoration: BoxDecoration(color: _ac.withOpacity(0.12), borderRadius: BorderRadius.circular(10)),
              ...
              Container(
                ...
                child: Column(children: tmp.keys.toList().asMap().entries.map((e) {
                  final coin = e.value; final isLast = e.key == tmp.length - 1;
                  return Column(children: [
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
                      child: Row(children: [
                        Container(width: 42, height: 42,
                          decoration: BoxDecoration(color: AppColors.warn.withOpacity(0.1), shape: BoxShape.circle,
                        ...
                        Expanded(child: TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: tmp[coin]["time"].toString(), hintStyle: const TextStyle(color: AppColors.textSub),
                            suffixText: "min", suffixStyle: const TextStyle(color: AppColors.textSub, fontSize: 12),
                          ...
                          onChanged: (v) { tmp[coin]["time"] = int.tryParse(v) ?? tmp[coin]["time"]; tmp[coin]["enabled"] = true; },
                        )),
                      ]),
                    ),
              ...
              SizedBox(width: double.infinity, height: 44,
                child: ElevatedButton.icon(
                  style: ElevatedButton.styleFrom(backgroundColor: AppColors.warn.withOpacity(0.15),
                    foregroundColor: AppColors.warn, elevation: 0,
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12),
                      side: BorderSide(color: AppColors.warn.withOpacity(0.4)))),
                  onPressed: () {
                    Map<String, Object> updates = {};
                    tmp.forEach((c, d) { updates["$c/time"] = d["time"]; updates["$c/enabled"] = d["enabled"]; });
                    _db.child("settings/coins").update(updates).then((_) {
                      if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(
                        content: const Text("✓ Coin settings saved."), backgroundColor: _alt));
                    });
                  },
              ...
              Container(
                ...
                decoration: BoxDecoration(color: _ac.withOpacity(0.06), borderRadius: BorderRadius.circular(10),
                  border: Border.all(color: _ac.withOpacity(0.2))),
                child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Icon(Icons.info_outline_rounded, color: _ac, size: 15), const SizedBox(width: 8),
                  const Expanded(child: Text(
                    "Both master and slave ESP32 will save the new network and reconnect automatically.",
              ...
              if (dlgScanning)
                Container(
                  ...
                  decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: AppColors.divider)),
                  child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: _ac)),
                    const SizedBox(width: 10),
                    const Text("Scanning…", style: TextStyle(color: AppColors.textSub, fontSize: 13)),
                  ]))
              else if (dlgSSIDs.isNotEmpty)
                Container(
                  decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: _ac.withOpacity(0.4))),
                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 2),
                  child: DropdownButtonHideUnderline(child: DropdownButton<String>(
                    value: dlgSSID, isExpanded: true, dropdownColor: AppColors.card,
                    icon: const Icon(Icons.expand_more, color: AppColors.textSub),
                    items: dlgSSIDs.map((s) => DropdownMenuItem(value: s,
                      child: Row(children: [Icon(Icons.wifi_rounded, color: _ac, size: 16), const SizedBox(width: 8),
                        Text(s, style: const TextStyle(color: AppColors.textPrim, fontSize: 13))]))).toList(),
                    onChanged: (v) => setDlg(() => dlgSSID = v),
                  )))
              else
                SizedBox(width: double.infinity, height: 44,
                  child: OutlinedButton.icon(
                    style: OutlinedButton.styleFrom(foregroundColor: _ac,
                      side: BorderSide(color: _ac.withOpacity(0.4)),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                    onPressed: () => scanDlgWifi(setDlg),
                    icon: const Icon(Icons.wifi_find_rounded, size: 17),
                    label: const Text("Scan Nearby Networks", style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                  )),

              // Rescan link when list is already showing
              if (dlgSSIDs.isNotEmpty && !dlgScanning) ...[
                const SizedBox(height: 4),
                Align(alignment: Alignment.centerRight,
                  child: TextButton.icon(
                    ...
                    style: TextButton.styleFrom(foregroundColor: AppColors.textSub, padding: EdgeInsets.zero),
                  )),
              ],
              const SizedBox(height: 10),
              ...
              TextField(
                ...
                style: const TextStyle(color: AppColors.textPrim, fontSize: 14),
                decoration: InputDecoration(
                  labelText: "WiFi Password", labelStyle: const TextStyle(color: AppColors.textSub, fontSize: 13),
              ...
              SizedBox(width: double.infinity, height: 48,
                child: ElevatedButton.icon(
                  style: ElevatedButton.styleFrom(backgroundColor: _ac.withOpacity(0.15),
                    foregroundColor: _ac, elevation: 0,
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12),
                      side: BorderSide(color: _ac.withOpacity(0.4)))),
                  onPressed: isSending ? null : () => sendWifi(setDlg, ctx),
                  icon: isSending
                    ? SizedBox(width: 15, height: 15, child: CircularProgressIndicator(strokeWidth: 2, color: _ac))
              ...
              SizedBox(width: double.infinity, height: 44,
                child: ElevatedButton.icon(
                  ...
                  label: const Text("Reset Machine WiFi", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                  onPressed: () {
                    showDialog(context: ctx, builder: (confirm) => AlertDialog(
                      backgroundColor: AppColors.card,
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
                      title: const Row(children: [
                        Icon(Icons.warning_amber_rounded, color: AppColors.danger, size: 22), SizedBox(width: 8),
                        Text("Reset Machine WiFi?", style: TextStyle(color: AppColors.textPrim, fontWeight: FontWeight.w700, fontSize: 15)),
                      ]),
                      content: const Text(
                        "This will erase the saved WiFi credentials on the ESP32 and reboot it into Setup Mode.\n\nYou will need to reconnect via the onboarding screen.",
                        style: TextStyle(color: AppColors.textSub, fontSize: 13, height: 1.5)),
                      actions: [
                        TextButton(onPressed: () => Navigator.pop(confirm),
                        ...
                        ElevatedButton.icon(
                          ...
                          label: const Text("Yes, Reset", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
                          onPressed: () async {
                            Navigator.pop(confirm);
                            try {
                              await _db.child("commands/wifi_reset").set({"pending": true});
                              if (ctx.mounted) Navigator.pop(ctx);
                              if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                                content: Text("✓ Reset command sent! Machine will reboot into Setup Mode."),
                                backgroundColor: AppColors.danger, duration: Duration(seconds: 4)));
                            } catch (e) {
                              if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(
                    ...
                    child: Column(children: [
                      AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: selected ? [BoxShadow(color: _tAccents[i].withOpacity(0.55), blurRadius: 14)] : [],
                        ),
...
class LogsScreen extends StatelessWidget {
          ...
          builder: (context, AsyncSnapshot<DatabaseEvent> snap) {
            if (!snap.hasData || snap.data!.snapshot.value == null)
              return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                _glowIcon(Icons.receipt_long_rounded, AppColors.textSub), const SizedBox(height: 12),
              ...
              children: [
                ...dates.map((date) {
                  ...
                  return Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        Row(children: [const Icon(Icons.calendar_today_rounded, color: AppColors.textSub, size: 14),
                          const SizedBox(width: 6),
                          Text(date.toString(), style: const TextStyle(color: AppColors.textPrim, fontWeight: FontWeight.w700))]),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                          decoration: BoxDecoration(color: _alt.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
                          child: Text("₱ ${total.toStringAsFixed(2)}",
                            style: TextStyle(color: _alt, fontWeight: FontWeight.bold, fontSize: 13))),
                      ]),
                      const SizedBox(height: 10),
                      const Divider(color: AppColors.divider, height: 1),
                      const SizedBox(height: 10),
                      Row(children: [_logChip("Charging", charge, _ac), const SizedBox(width: 8),
                ...
                Padding(
                  ...
                  child: SizedBox(width: double.infinity, height: 50,
                    child: ElevatedButton.icon(
                      style: ElevatedButton.styleFrom(backgroundColor: AppColors.danger.withOpacity(0.15),
                        foregroundColor: AppColors.danger, elevation: 0,
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14),
  ...
  void _confirmClear(BuildContext ctx, DatabaseReference logRef) {
    showDialog(context: ctx, builder: (dlg) => AlertDialog(
      backgroundColor: AppColors.card,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
      title: const Row(children: [Icon(Icons.warning_amber_rounded, color: AppColors.danger, size: 22), SizedBox(width: 8),
